...
const Detalle = () => {
  const { id } = useParams(); // Obtener los parámetros de la URL
    ...
    <div className='m-auto  pt-[70px] md:pt-[90px] '>
        <div className='w-full h-16  bg-celeste flex justify-between items-center p-4'>
                   <div className='w-full max-w-[1240px] flex justify-between items-center m-auto'>
                        <h1 className='w-full m-auto  md:text-2xl font-semibold text-azul'>{producto.name}</h1>
                        <button onClick={back} className='text-center border rounded-lg shadow-xl m-auto bg-celeste hover:bg-azul '><FaLongArrowAltLeft className='m-auto text-xl text-white' /></button>
                   </div>
        </div>
        <div className='m-auto max-w-[1240px] mt-2 p-4'>
              ...
              <div className='galeria '>
                ...
                {galeria.map((imagen, index) => {
                  return (
                    ...
                    <img src={`http://localhost:3000${imagen.url}`} alt={`imagen ${imagen.ur}`}  key={imagen.url} onClick={()=>{ ImageClick(index)}} className='w-16 h-10 xs:h-12  sm:w-20 sm:h-16 md:w-40 md:h-20 hover:opacity-80 cursor-pointer  m-auto mx-0 relative'/>
                    
                  )
                  
                })}
              </div>
              <p className='absolute -bottom-6 right-0 cursor-pointer hover:text-gray-500' onClick={mostrar}>ver mas</p>
            </div>
            ...
            <div className='mt-20'>
                <h2 className='text-2xl text-azul font-semibold'>{producto.location}</h2>
